...
public class PodjetjeController {
    ...
    public ResponseEntity<Podjetje> updatePodjetje(@PathVariable long id,@RequestBody Podjetje podjetjeDetails) {
        Optional<Podjetje> updatePodjetje = podjetjeDao.findById(id);

        Podjetje vao = updatePodjetje.get();
        podjetjeDao.save(podjetjeDetails);

        return ResponseEntity.ok(vao);
    }

    @DeleteMapping("/delete/{id}")
    public ResponseEntity<Podjetje> deletePodjetje(@PathVariable("id") long id) {
        Optional<Podjetje> val = podjetjeDao.findById(id);

        Podjetje vao=val.get();
        podjetjeDao.delete(vao);
